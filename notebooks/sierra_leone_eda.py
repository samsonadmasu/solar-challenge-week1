...
# # Exploratory Data Analysis for Sierra Leone Solar Data
# 
# This notebook performs exploratory data analysis on solar farm data from Sierra Leone.
...
# Load the Sierra Leone solar data
# Replace with your actual file path
file_path = '../data/sierra_leone_data.csv'
...
try:
    ...
    df = pd.DataFrame({
        ...
        'ModA': np.random.normal(430, 95, 1000),
        'ModB': np.random.normal(440, 100, 1000),
        'Tamb': np.random.normal(27, 4, 1000),
        'RH': np.random.uniform(40, 95, 1000),
        'WS': np.random.exponential(2.5, 1000),
        'WSgust': np.random.exponential(4.5, 1000),
        'WSstdev': np.random.uniform(0.4, 1.8, 1000),
        'WD': np.random.uniform(0, 360, 1000),
        'WDstdev': np.random.uniform(4, 18, 1000),
        'BP': np.random.normal(1010, 6, 1000),
        'Cleaning': np.random.choice([0, 1], 1000, p=[0.93, 0.07]),
        'Precipitation': np.random.exponential(0.2, 1000),
        'TModA': np.random.normal(32, 7, 1000),
        'TModB': np.random.normal(33, 7, 1000),
    })
...
if 'Cleaning' in df_clean.columns:
    ...
    plt.xlabel('Cleaning')
    plt.ylabel('Average Module Reading (W/mu00b2)')
...
if all(col in df_clean.columns for col in ['WS', 'GHI']):
    ...
    if 'RH' in df_clean.columns and 'Tamb' in df_clean.columns:
        ...
        plt.ylabel('Ambient Temperature (u00b0C)')
        plt.grid(True)
...
if all(col in df_clean.columns for col in ['RH', 'Tamb', 'GHI']):
    ...
    ax1.set_ylabel('Ambient Temperature (u00b0C)')
    ax1.grid(True)
    cbar = plt.colorbar(scatter, ax=ax1)
    cbar.set_label('GHI (W/mu00b2)')
    ...
    ax2.set_ylabel('Ambient Temperature (u00b0C)')
...
if all(col in df_clean.columns for col in ['GHI', 'Tamb']):
    ...
    plt.xlabel('Ambient Temperature (u00b0C)')
    plt.ylabel('GHI (W/mu00b2)')
    plt.grid(True)
...
print("\nKey Findings from Sierra Leone Solar Data EDA:")
